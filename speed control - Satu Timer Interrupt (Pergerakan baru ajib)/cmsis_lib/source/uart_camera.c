...
//	}
	...
	if( USART_GetITStatus(USART3, USART_IT_RXNE) )
	{
		...
		if(uartRxBuff[0] == 0x58 || uartRxBuff[0] == 0x59 || uartRxBuff[0] == 0x54) //0x58 = X   0x59 = Y   0x54 = T
		{
			rx_index = 4;
			counter++;
		}
		...
		if(counter == rx_index)
		{
			...
			{
				compassHeading = (float) 1000 * (uartRxBuff[1] - 0x30) + 100 * (uartRxBuff[2] - 0x30) + 10 * (uartRxBuff[3] - 0x30) + (uartRxBuff[4] - 0x30);
