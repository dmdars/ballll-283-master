...
volatile uint16_t tra_speed_count_x = 0;
volatile int16_t tra_speed_manual_count_x = 0; //manual trajectory
volatile int16_t tra_speed_manual_count_chase_x = 0; //pengejar
volatile int16_t tra_speed_manual_target_x = 0; //target count
volatile uint8_t is_tra_speed_x_done = 0;
float tra_speed_input_x;
double tra_speed_count_total_accel_x;
double tra_speed_current_x;
double tra_speed_current_time_x;
uint8_t tra_x_speed_phase = 0;
uint8_t tra_x_speed_reverse = 0;

double tra_speed_count_total_constant_x;

double tra_int_current_x = 0.0;
double tra_speed_current_x_prev = 0.0;

volatile uint16_t tra_speed_count_y = 0;
volatile int16_t tra_speed_manual_count_y = 0; //manual trajectory
volatile int16_t tra_speed_manual_count_chase_y = 0;
volatile int16_t tra_speed_manual_target_y = 0;
volatile uint8_t is_tra_speed_y_done = 0;
float tra_speed_input_y;
double tra_speed_count_total_accel_y;
double tra_speed_current_y;
double tra_speed_current_time_y;
uint8_t tra_y_speed_phase = 0;
uint8_t tra_y_speed_reverse = 0;

double tra_speed_count_total_constant_y;

double tra_int_current_y = 0.0;
double tra_speed_current_y_prev = 0.0;
...
int is_counted_up = 0;
int move_ccounter = 1;
volatile uint8_t is_mode_automatic = 1;
uint32_t tim2_enable_flag=0;
volatile uint8_t calc_flag = 0;
//end variable trajectory

volatile uint8_t is_tra_done_changeflag = 0;
volatile uint8_t is_step_move_done = 0;
uint8_t step_move_counter = 1;
//step move variable

volatile uint32_t time_counter = 0;
uint32_t time_counter_prev = 0;
uint8_t time_counter_delta = 0;
//time variable
...
void traInit(double x, double y, double w, double time, double time_constant, int movement_type) {
	...
	traXInit(x, time, time_constant, movement_type);
	traYInit(y, time, time_constant, movement_type);
	traWInit(w, time, movement_type);
}
...
void traXInit (float v, double time_accel_x, double time_constant_x, int movement_type)
{
	...
	resetInit();
	tra_x_speed_phase = 1;
	tra_speed_count_x = 0;							//nol kan counter trajectory
	move_type = movement_type;
	tra_speed_input_x = v;							//input total trajectory yang dimau
	tra_speed_count_total_accel_x = time_accel_x / time_sampling;	//hitung total waktu yang dibutuhkan untuk trajectory
	tra_speed_current_x = 0.0;						//nol kan hasil trajectory
	is_tra_speed_x_done = 0;							//reset tra status

	tra_speed_count_total_constant_x = time_constant_x / time_sampling;
}
...
void traYInit(float v, double time_accel_y, double time_constant_y, int movement_type) {
	...
	resetInit();
	tra_y_speed_phase = 1;
	tra_speed_count_y = 0;							//nol kan counter trajectory
	move_type = movement_type;
	tra_speed_input_y = v;							//input total trajectory yang dimau
	tra_speed_count_total_accel_y = time_accel_y / time_sampling;	//hitung total waktu yang dibutuhkan untuk trajectory
	tra_speed_current_y = 0.0;						//nol kan hasil trajectory
	is_tra_speed_y_done = 0;							//reset tra status

	tra_speed_count_total_constant_y = time_constant_y / time_sampling;
}
...
//	 			calc_flag = 0;
	 			...
	 			time_counter_delta = time_counter - time_counter_prev;
	 			time_counter_prev = time_counter;

//	 			calcSpeed(time_counter_delta);
//	 			calcPos(time_counter_delta);
				calculatePosition2(time_counter_delta);
				calcTrj();
				calcTrjSpeed(time_counter_delta);
				calculatePID(time_counter_delta);
				kinematic();
				if (is_tra_done_changeflag == 1)
				{
					for(int i = 0; i<10 ;i++){
						output = 0;
						outputDua = 0;
...
void calcTimingX(float x, float v_max, float a_max, float j_max){
	double v_now_1 = 0;
	double v_now_2 = 0;
	double accel_time = 0;
	double accel_time_2 = 0;

	if (x < 0) {
		tra_x_speed_reverse = 1;
		x = -x;
	}else {
		tra_x_speed_reverse = 0;
	}

	double v_1_scale = sqrt((x*a_max)/(1.875*pow(v_max,2.0))); //hitung pen-skala kecepatan apabila jarak terlalu dekat kecepatan maksimum harus diturunkan dengan skala
	double v_2_scale = sqrt((x*j_max*2)/(5.773502691896*pow(v_max,2.0)));

	if (v_1_scale < 1) {
		v_now_1 = v_max*v_1_scale;
		accel_time = (1.875*v_now_1)/a_max; //hitung waktu akselerasi dengan skala
	} else {
		v_now_1 = v_max;
		accel_time = (1.875*v_max)/a_max; //hitung waktu akselerasi tanpa skala
	}

	if (v_2_scale < 1) {
		v_now_2 = v_max*v_2_scale;
		accel_time_2 = (5.773502691896*v_now_2)/(j_max*2); //hitung waktu akselerasi dengan skala dengan mempertimbangkan jerk
	} else {
		v_now_2 = v_max;
		accel_time_2 = (5.773502691896*v_max)/(j_max*2); //hitung waktu akselerasi tanpa skala dengan mempertimbangkan jerk
	}

	if (accel_time_2 > accel_time) {
		accel_time = accel_time_2; //apabila limit jerk tercpai maka waktu jerk lebih lama dan akan digunakan
		v_now_1 = v_now_2;
	}

	double constant_time = (x/v_now_1) - accel_time; //apabila jarak cukup jauh akan ada waktu selama kecepatan maksimum

	traXInit(v_now_1, accel_time, constant_time, move_type);

}

void calcTimingY(float y, float v_max, float a_max, float j_max){
	double v_now_1 = 0;
	double v_now_2 = 0;
	double accel_time = 0;
	double accel_time_2 = 0;
	...
	if (y < 0) {
		tra_y_speed_reverse = 1;
		y = -y;
	}else {
		tra_y_speed_reverse = 0;
	}

	double v_1_scale = sqrt((y*a_max)/(1.875*pow(v_max,2.0))); //hitung pen-skala kecepatan apabila jarak terlalu dekat kecepatan maksimum harus diturunkan dengan skala
	double v_2_scale = sqrt((y*j_max*2)/(5.773502691896*pow(v_max,2.0)));
	...
	double constant_time = (y/v_now_1) - accel_time; //apabila jarak cukup jauh akan ada waktu selama kecepatan maksimum

	traYInit(v_now_1, accel_time, constant_time, move_type);
}

void calcTrj(){
	if (tra_x_speed_phase == 1) {
		//akselerasi
		if (tra_speed_count_x < tra_speed_count_total_accel_x){
			if (tra_speed_count_total_accel_x > 0.0){
				tra_speed_current_time_x = tra_speed_count_x / tra_speed_count_total_accel_x; //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_x = tra_speed_input_x * ((6 * pow(tra_speed_current_time_x,5.0)) - (15 * pow(tra_speed_current_time_x,4.0)) + (10 * pow(tra_speed_current_time_x,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_count_x >= tra_speed_count_total_accel_x) {
			tra_x_speed_phase = 2;
			tra_speed_count_x = 0;
		}
	} else if (tra_x_speed_phase == 2) {
		//constant speed
		if (tra_speed_count_x < tra_speed_count_total_constant_x) {
			if (tra_speed_count_total_constant_x > 0.0){
				tra_speed_current_x = tra_speed_input_x;
			}
		} else if (tra_speed_count_x >= tra_speed_count_total_constant_x) {
			tra_x_speed_phase = 3;
			tra_speed_count_x = 0;
		}
	} else if (tra_x_speed_phase == 3) {
		//dekselerasi
		if (tra_speed_count_x < tra_speed_count_total_accel_x){
			if (tra_speed_count_total_accel_x > 0.0){
				tra_speed_current_time_x = 1.0 - (tra_speed_count_x / tra_speed_count_total_accel_x); //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_x = tra_speed_input_x * ((6 * pow(tra_speed_current_time_x,5.0)) - (15 * pow(tra_speed_current_time_x,4.0)) + (10 * pow(tra_speed_current_time_x,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_count_x >= tra_speed_count_total_accel_x) {
			tra_speed_current_x = 0.0;
			is_tra_speed_x_done = 1;
		}
	} else if (tra_x_speed_phase == 4) {
		//untuk mode manual
		if (tra_speed_manual_count_chase_x < 0) {
			tra_speed_manual_count_x = -tra_speed_manual_count_chase_x;
			tra_x_speed_reverse = 1;
		}else {
			tra_speed_manual_count_x = tra_speed_manual_count_chase_x;
			tra_x_speed_reverse = 0;
		}
		if (tra_speed_manual_count_x < tra_speed_count_total_accel_x){
			if (tra_speed_count_total_accel_x > 0.0){
				tra_speed_current_time_x = tra_speed_manual_count_x / tra_speed_count_total_accel_x; //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_x = tra_speed_input_x * ((6 * pow(tra_speed_current_time_x,5.0)) - (15 * pow(tra_speed_current_time_x,4.0)) + (10 * pow(tra_speed_current_time_x,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_manual_count_x >= tra_speed_count_total_accel_x) {
			//do nothing
		}
	}
	...
	if (tra_y_speed_phase == 1) {
		if (tra_speed_count_y < tra_speed_count_total_accel_y){
			if (tra_speed_count_total_accel_y > 0.0){
				tra_speed_current_time_y = tra_speed_count_y / tra_speed_count_total_accel_y; //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_y = tra_speed_input_y * ((6 * pow(tra_speed_current_time_y,5.0)) - (15 * pow(tra_speed_current_time_y,4.0)) + (10 * pow(tra_speed_current_time_y,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_count_y >= tra_speed_count_total_accel_y) {
			tra_y_speed_phase = 2;
			tra_speed_count_y = 0;
		}
	} else if (tra_y_speed_phase == 2) {
		if (tra_speed_count_y < tra_speed_count_total_constant_y) {
			if (tra_speed_count_total_constant_y > 0.0){
				tra_speed_current_y = tra_speed_input_y;
			}
		} else if (tra_speed_count_y >= tra_speed_count_total_constant_y) {
			tra_y_speed_phase = 3;
			tra_speed_count_y = 0;
		}
	} else if (tra_y_speed_phase == 3) {
		if (tra_speed_count_y < tra_speed_count_total_accel_y){
			if (tra_speed_count_total_accel_y > 0.0){
				tra_speed_current_time_y = 1.0 - (tra_speed_count_y / tra_speed_count_total_accel_y); //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_y = tra_speed_input_y * ((6 * pow(tra_speed_current_time_y,5.0)) - (15 * pow(tra_speed_current_time_y,4.0)) + (10 * pow(tra_speed_current_time_y,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_count_y >= tra_speed_count_total_accel_y) {
			tra_speed_current_y = 0.0;
			is_tra_speed_y_done = 1;
		}
	} else if (tra_y_speed_phase == 4) {
		if (tra_speed_manual_count_chase_y < 0) {
			tra_speed_manual_count_y = -tra_speed_manual_count_chase_y;
			tra_y_speed_reverse = 1;
		}else {
			tra_speed_manual_count_y = tra_speed_manual_count_chase_y;
			tra_y_speed_reverse = 0;
		}
		if (tra_speed_manual_count_y < tra_speed_count_total_accel_y){
			if (tra_speed_count_total_accel_y > 0.0){
				tra_speed_current_time_y = tra_speed_manual_count_y / tra_speed_count_total_accel_y; //cari waktu sekarang untuk dimasukan ke dalam rumus trajectory
				tra_speed_current_y = tra_speed_input_y * ((6 * pow(tra_speed_current_time_y,5.0)) - (15 * pow(tra_speed_current_time_y,4.0)) + (10 * pow(tra_speed_current_time_y,3.0))); //kalikan input trajectory dengan hasil rumus trajectory
			}
		} else if (tra_speed_manual_count_y >= tra_speed_count_total_accel_y) {
			//do nothing
		}
	}
...
void calcTrjSpeed(uint8_t time) {
	if (tra_x_speed_reverse == 0) {
		tra_int_current_x += (tra_speed_current_x + tra_speed_current_x_prev)* 0.5 * (time*time_sampling);
	}else {
		tra_int_current_x -= (tra_speed_current_x + tra_speed_current_x_prev)* 0.5 * (time*time_sampling);
	}
	tra_speed_current_x_prev = tra_speed_current_x;

	if (tra_y_speed_reverse == 0) {
		tra_int_current_y += (tra_speed_current_y + tra_speed_current_y_prev)* 0.5 * (time*time_sampling);
	}else {
		tra_int_current_y -= (tra_speed_current_y + tra_speed_current_y_prev)* 0.5 * (time*time_sampling);
	}

	tra_speed_current_y_prev = tra_speed_current_y;
}
...
//		if (is_step_move_done == 0) {
//			if (is_tra_speed_x_done == 1 && is_tra_speed_y_done == 1 && is_tra_w_done == 1) {
//				if (is_tra_done_changeflag ==0) {
//					step_move_counter++;
//					is_tra_done_changeflag = 1;
//
//				}
//			}
//		}
	//membedakan tipe gerakan
	//1. gerakan menggunakan tiga variabel x,y,w
	//2. gerakan menggunakan 2 variabel
	//3. gerakan menggunakan 1 variabel
		switch (move_type){
			...
			case 2:
				if (is_tra_speed_x_done == 1 && is_tra_speed_y_done == 1) {
							if (is_tra_done_changeflag ==0) {
								is_tra_done_changeflag = 1;
							}
				}
				else if (is_tra_speed_x_done == 1 && is_tra_w_done == 1) {
							if (is_tra_done_changeflag ==0) {
								is_tra_done_changeflag = 1;
							}
				}
				else if (is_tra_speed_y_done == 1 && is_tra_w_done == 1) {
							if (is_tra_done_changeflag ==0) {
								is_tra_done_changeflag = 1;
							}
				}
				break;
			case 3:
				if (is_tra_speed_x_done == 1 && is_tra_speed_y_done == 0) {
							if (is_tra_done_changeflag ==0) {
								is_tra_done_changeflag = 1;
								is_tra_speed_x_done = 0;
								tra_x_speed_phase = 0;
							}
				}
				if (is_tra_speed_y_done == 1 && is_tra_speed_x_done == 0) {
							if (is_tra_done_changeflag ==0) {
								is_tra_done_changeflag = 1;
								is_tra_speed_y_done = 0;
								tra_y_speed_phase = 0;
							}
				}
//				else if (is_tra_w_done == 1 && is_tra_speed_x_done == 0 && is_tra_speed_y_done == 0) {
//							if (is_tra_done_changeflag ==0) {
//								is_tra_done_changeflag = 1;
//								is_tra_w_done = 0;
//							}
//				}
		...
		if (is_tra_speed_x_done == 0) {
			tra_speed_count_x++;
		}

		if (is_tra_speed_y_done == 0) {
			tra_speed_count_y++;
		}

//		if (is_tra_w_done == 0 && move_type == 1) {
//				tra_count_w++;
//		}
...
//	current_speed_6 = (100 * data6.Diff * 1.1) / (time_sampling * 1326);
	...
	current_speed_1 = wheel_radius *(((2.0 * PI / ppr) * data2.Diff) / (time * time_sampling));
	current_speed_2 = wheel_radius *(((2.0 * PI / ppr) * data.Diff) / (time * time_sampling));
	current_speed_3 = wheel_radius *(((2.0 * PI / ppr) * data4.Diff) / (time * time_sampling));
	current_speed_4 = wheel_radius *(((2.0 * PI / ppr) * data3.Diff) / (time * time_sampling));
...
//	odo_local_w = (current_speed_5+current_speed_6) * 1.3554 / (2*robot_radius);
	...
	current_x += (odo_global_vx + odo_global_vx_prev)* 0.5 * (time * time_sampling);
	current_y += (odo_global_vy + odo_global_vy_prev)* 0.5 * (time * time_sampling);
	current_w += (odo_global_w + odo_global_w_prev)* 0.5 * (time * time_sampling); //* (180 / PI);
...
void calculatePID(uint8_t time){
	pid_error_x = tra_int_current_x - current_x;								//hitung error pid
	pid_proportional_x = pid_error_x * KP0;									//hitung proportional output
	pid_integral_x += KI0 * (pid_error_x * (time*time_sampling));					//hitung integral output
	pid_derivative_x = KD0 * ((tra_int_current_x - last_tra_x) / (time*time_sampling));	//hitung derivative output
	pid_current_x = pid_proportional_x + pid_integral_x - pid_derivative_x;	//hitung total pid
	last_tra_x = tra_int_current_x;												//simpan trajectory sekarang untuk perhitungan setelah ini


	pid_error_y = tra_int_current_y - current_y;
	pid_proportional_y = pid_error_y * KP0;
	pid_integral_y += KI0 * (pid_error_y * (time*time_sampling));
	pid_derivative_y = KD0 * ((tra_int_current_y - last_tra_y) / (time*time_sampling));
	pid_current_y = pid_proportional_y + pid_integral_y - pid_derivative_y;
	last_tra_y = tra_int_current_y;


	pid_error_w = tra_current_w - current_w;
	pid_proportional_w = pid_error_w * KP_W;
	pid_integral_w += KI_W * (pid_error_w * (time*time_sampling));
	pid_derivative_w = KD_W * ((tra_current_w - last_tra_w) / (time*time_sampling));
...
void stepMove() {
	if (is_tra_done_changeflag == 1) {
		switch (step_move_counter) {
			case 1:
				calcTimingX(9000.0,2600.0,1600.0,3200.0);
				is_tra_done_changeflag = 0;
				break;
			case 2:
				calcTimingX(9000.0,2600.0,1600.0,3200.0);
