...
void findBall(){
	...
	if(xCoorMid>xCoor + 10){
		...
		if(rotate_flag == 0)
		{
			traWInit(0,0);
			int temp = 0;
			while((xCoor < 350 && xCoor > 230) && (yCoor < 350 && yCoor > 340)) // get ball near handle
			{
				if(temp == 0)
				{
					handleRotateIn();
				...
				if((xCoor < 290 && xCoor > 280) && (yCoor < 400 && yCoor > 320)) // get ball in handle
				{
					obtain_ball = 1;
					break;
				}
		...
		if(rotate_flag == 0)
		{
			traWInit(0,0);
			int temp = 0;
			while((xCoor < 350 && xCoor > 230) && (yCoor < 400 && yCoor > 320)) // get ball near handle
			{
				if(temp == 0)
				{
					handleRotateIn();
				...
				if((xCoor < 290 && xCoor > 280) && (yCoor < 350 && yCoor > 340)) // get ball in handle
				{
					obtain_ball = 1;
					break;
				}
...
//		{
			...
			Delayms(2800);
			traInit(0, 100, 0, 0);//ngatur PID di speed_control.h
			Delayms(4000);
			resetInit();
			traInit(0, 0, 0, 0);
			flagStage = 1;
...
//				if (obtain_ball == 1){
					handleRotateIn();
					traInit(-100,0,0,0);			// handle liat di mekanisme penendang_penendang.c
					Delayms(3000);
					flagStage = 2;
//					break;
//				}
//				else if (obtain_ball == 0)
//				{
//					moveInput();
//					flagStage = 1;
//				}
